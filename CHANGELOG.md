...
- 추가: 상단 고정 세션 표시 영역에 `세션 저장` 버튼 추가
- 추가: 상단 고정 세션 표시 영역에 오늘 목표 남은 시간/완료 상태 표시
- 추가: 설정 화면에 `앱 강제 업데이트` 버튼 추가
- 개선: 강제 업데이트 시 FocusManager 캐시만 삭제하고 localStorage/IndexedDB 데이터는 유지한 채 새로고침
- 개선: 세션 요약에서 공부 메타정보가 중복 표시되던 부분 정리
- 유지: 기존 저장 키 `focus_manager_v1` 유지
- 유지: v1.3.2 상단 경과시간 표시, v1.3.1 저장 안정화, v1.3.0 공부 플래너, v1.2.x GitHub 백업 유지
- 갱신: service worker cache version `focusmanager-v1-3-3`로 변경

## v1.3.2

- 추가: 상단 고정 현재시간 배지에 세션 중 경과 시간 표시 추가
- 추가: 세션 중 남은 시간 표시 추가
- 추가: 목표 시간을 넘긴 경우 초과 시간 표시 추가
- 추가: 세션 종료 예정 시각 표시 추가
- 추가: 30분 단위 경과 상태 표시 추가
- 추가: 일시정지 중 상단 배지에 일시정지 상태 표시
- 개선: 스크롤 중에도 현재 세션 진행 상태를 확인할 수 있도록 고정 배지 확장
- 유지: 기존 저장 키 `focus_manager_v1` 유지
- 유지: v1.3.1 저장 안정화, v1.3.0 공부 플래너, v1.2.x GitHub 백업 유지
- 갱신: service worker cache version `focusmanager-v1-3-2`로 변경
